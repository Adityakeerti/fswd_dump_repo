...
import { Scan, X, CheckCircle, AlertCircle, Loader } from 'lucide-react'
...
interface ScanModalProps {
  ...
  title: string
  onClose: () => void
  onScanReceived: (bookId: string, bookInfo: any) => void
  children: React.ReactNode
  footer: React.ReactNode
}

export default function ScanModal({ open, title, onClose, onScanReceived, children, footer }: ScanModalProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.timestamp > lastTimestamp && data.book_id && data.book_info) {
          ...
          // Notify parent component with book ID as string
          onScanReceived(String(data.book_info.id), data.book_info)
      ...
      <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-400">Scan barcode from your phone</div>
              </div>
            </div>
            <button
        ...
        {scanMode === 'failed' && (
          <div className="p-4 rounded-2xl bg-rose-500/20 border border-rose-500/50 flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="text-sm font-semibold text-white mb-1">Scan timeout</div>
              <div className="text-xs text-slate-300">
                No scan received after 2 attempts. Falling back to manual entry...
              </div>
            </div>
          </div>
        )}
